...
{!! Form::open(['method'=>'get','route' => ['mimin.laporan.penjualan.laporan2'],'class'=>'form form-inline input-group']) !!}
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered mb-0">
                                        ...
                                        <tbody>
                                          ...
                                          <tr>
                                              <td>{{$pesanan->tanggal_pembayaran_date->format('d-m-Y')}}</td>
                                              <td>{{$pesanan->nama}}</td>
                                              <td class="text-right">{{$pesanan->total_kuantitas}}</td>
                                              <td>{{$pesanan->satuan}}</td>
                                              <td class="text-right">{{number_format($pesanan->harga_penjualan)}}</td>
                                              <td class="text-right">{{number_format($pesanan->total_penjualan)}}</td>

                                          </tr>
                                          @empty
                                          <tr><td colspan="6">Tidak ada penjualan dari {{$start_date->format("d-m-Y")}} hingga {{$end_date->format("d-m-Y")}}.</td></tr>
                                          @endforelse
                                        </tbody>
                                    </table>
                                </div>
